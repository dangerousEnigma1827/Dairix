...
// ────────────────────────────────────────────────────────────────────────────
// Demo data — replace with real delivery history from the API

const DEMO_ITEMS = [{ name: "Full Cream Milk", quantity: 2, unit: "L" }];

function buildMonth(monthOffset: number): DayEntry[] {
  ...
  return Array.from({ length: daysInMonth }, (_, i) => {
    ...
    // Deterministic fake pattern for demo — every 6th day skipped, rest delivered
    const status: DailyStatus = isFuture ? "pending" : (i + 1) % 6 === 0 ? "skipped" : "delivered";

    return {
      date: d,
      status,
      deliveredAt: status === "delivered" ? `${6 + (i % 2)}:${i % 2 === 0 ? "10" : "42"} AM` : null,
      items: status === "delivered" ? DEMO_ITEMS : [],
    };
  });
}
...
export default function DeliveryTracker() {
  ...
  const days = useMemo(() => buildMonth(monthOffset), [monthOffset]);
  ...
  const stats = useMemo(() => {
    const past = days.filter((d) => d.status !== "pending");
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right shrink-0">
                        ...
                        {d.deliveredAt && (
                          <p className="text-[11px] text-slate-400">{d.deliveredAt}</p>
                        )}
                      </div>
                    </div>
                  );
